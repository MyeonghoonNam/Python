...
# id vs __eq__ (==) 증명
x = {'name': 'kim', 'age': 33, 'city': 'Seoul', 'class': 10}
y = x
...
print('EX2-6 -', x, z)
print('EX2-7 -', x is z) # 같은 객체이면 True
print('EX2-8 -', x is not z) 
print('EX2-9 -', x == z) # 값이 같으면 True

# 객체 생성 후 완전 불변(id) -> 즉, id는 객체 주소(정체성)비교, == (__eq__) 는 값 비교
...
print('EX3-3 -', tuple1 == tuple2) # 아래와 같은 기능
...
print('EX4-8 -', tl1)
print('EX4-9 -', tl2) # 튜플 재 할당(객체 새로 생성) -> 튜플은 불변형이기 때문에
print('EX4-10 -', tl3)
print(id(tl1[0]))
print(id(tl3[0]))
print(id(tl1[1]))
...
print(id(tl1)) # 재할당된 객체주소로 둘 다 변환되었다.
print(id(tl2))
print(id(tl3[2]))
...
print('EX5-1 -', id(basket1), basket1, id(basket2), basket2, id(basket3))
...
print('EX6-2 -', mul(a, b), a, b) # 가변형 a -> 원본 데이터 변경 즉, 주소를 넘기기 때문에 변경 -> 주소를 넘긴다는 복사를 한 x를 뜻하게 된다.
